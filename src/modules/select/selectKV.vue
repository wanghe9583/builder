...
export default {
    ...
    computed: {
        ...
        strCopyDetail() {
            return `
            <label>{{${this.x_prefix.detail}${this.model.key}?${this.x_prefix.detail}${this.model.key}.v:null}}</label>
             `
        },
        strCopyAdd() {
            let e = this.config;
    ...
    methods: {
                ...
                modules: {
                    ...
                    script: {
                        data: {
                            ...
                            columns: `{title: "${this.model.title}", align: "center",width: 200, render: (h, p) => h("label", p.row.${this.model.key}?p.row.${this.model.key}.v:null) },`
                        },
                        created: {
                            beforeShow: null,
                            beforeEditShow: `if (obj.${this.model.key}) obj.${this.model.key} = this.ui.dics.${this.model.firstUpeer}.find(x => x.k == obj.${this.model.key}.k);`
                        }
                    }
                },
                main: {
        ...
        setStr() {
            ...
            this.setModel();
            modulesJS.modules("selectKVModule", Vue, this.str)
        }
    },
    mounted() {
        this.setModel();
